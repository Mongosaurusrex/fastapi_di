...
from .repositories import UserNotFoundError, UserRepository, NotFoundError
...
def test_add(_, client):
    repository_mock = mock.Mock(spec=UserRepository)
    repository_mock.add_user.return_value = User(
        id=1,
        email="xyz@email.com",
        hashed_password="pwd",
        is_active=True,
    ...
    repository_mock.add_user.assert_called_once_with(
...
def test_remove(client):
    ...
    repository_mock.delete_user_by_id.assert_called_once_with(1)


def test_remove_404(client):
    repository_mock = mock.Mock(spec=UserRepository)
    repository_mock.delete_user_by_id.side_effect = UserNotFoundError(1)
